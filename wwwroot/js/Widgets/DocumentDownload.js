(function (component) {
    ...
    function init() {
        $('.contentDownload').each(function () {
            $(this).find('.dropdown-menu button').each(function () {
                ...
                $(this).on('click', function (e) {
                    ...
                    self.find('[data-file-date] span:not(.lineOrizontal)').each(function () {
                        if ($(this).attr('data-culture') != culture)
                            $(this).hide();
                        else
                            $(this).show();
                    });
                    ...
                    self.find('.download-actions ul li > button > span').each(function () {
                        if ($(this).attr('data-culture') != culture)
                            $(this).hide();
                        else
                            $(this).show();
                    });
                    self.find('.download-actions ul .dropdown-menu button').each(function () {
                        if ($(this).attr('data-culture') == culture)
                            $(this).hide();
                        else
                            $(this).show();
    ...
    function sendRemote(self) {
       ...
       var category = self.attr('data-category'); //parseInt(self.attr('data-category'), 10);
        var folder = self.attr('data-folder');
       $.get('/download/readmore?page=' + page + '&pageSize=' + pageSize + '&year=' + year + '&category=' + category + '&folder=' + folder, function (data) {
          
            var count = (data.match(/row/g) || []).length;
            if (page > 0)
                self.closest('.document-list').find('[data-type="docscontainer"]').append(data);
            else
                self.closest('.document-list').find('[data-type="docscontainer"]').html(data);
            ...
            if (count >= pageSize) {
                ...
                self.closest('.document-list').find('[data-readmore-container="docs"]').addClass('d-none');
